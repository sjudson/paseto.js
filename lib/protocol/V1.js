...
function __encrypt(data, key, footer, nonce, cb) {
  ...
  const header = utils.header.local(self);
...
function decrypt(token, key, footer, cb) {
  ...
  if (!footer) {
    footer = utils.footer.extract(token);
    token  = utils.footer.remove(token);
  } else {
    [ footer ] = (utils.parse('utf-8'))(footer);
    token      = utils.footer.validate(token, footer);
  }

  const header = utils.header.local(self);

  let payload = utils.header.validate(token, header);
...
function sign(data, key, footer, cb) {
  ...
  const header = utils.header.public(self);
