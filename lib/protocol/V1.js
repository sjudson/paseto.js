...
function aeadDecrypt(key, prefix, token, footer) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const nlen  = self.constants.NONCE_SIZE;
    const nonce = Buffer.from(decoded).slice(0, nlen);

    // decrypt and verify

    const ciphertext = Buffer.from(decoded).slice(nlen, mlen - self.constants.MAC_SIZE);
    const mac        = Buffer.from(decoded).slice(mlen - self.constants.MAC_SIZE);
    ...
    key.split(nonce.slice(0, 16), (err, keys) => {
      ...
      const decryptor  = crypto.createDecipheriv(self.constants.CIPHER_MODE, enckey, nonce.slice(16, 32));
