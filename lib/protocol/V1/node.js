...
function _nodeV1() {
  ...
  this._constants[ 'CIPHER_MODE' ] = 'aes-256-ctr';
  this._constants[ 'HASH_ALGO' ]   = 'sha384';
  this._constants[ 'NONCE_SIZE' ]  = 32;
  this._constants[ 'MAC_SIZE' ]    = 48;
}
_nodeV1.super_ = V1;
...
function pk(cb) {
  ...
  const pk = new PrivateKey(new _nodeV1());
 ...
 * @returns {Callback|Promise}
 */
_nodeV1.prototype.symmetric = sk;
function sk(cb) {
  const done = utils.ret(cb);

  // minor hack to mimic php api without circular dependency - probably a better way to do this
  const SymmetricKey = require('../../key/symmetric');
  const sk = new SymmetricKey(new _nodeV1());
 ...
 * @returns {Number}
 */
_nodeV1.prototype.sklength = sklength;
function sklength() {
 ...
 */
_nodeV1.prototype.__encrypt = __encrypt;
 ...
 * @returns {Function|Callback}
 */
_nodeV1.prototype.encrypt = encrypt;
 ...
 * @returns {Function|Callback}
 */
_nodeV1.prototype.decrypt = decrypt;
 ...
 * @returns {String} pk
 */
_nodeV1.prototype.public = gpublic;
function gpublic(sk) {
  const ko = crypto.createPublicKey({ key: sk, format: 'pem' });
  return ko.export({ type: 'pkcs1', format: 'pem' });
}
 ...
 * @returns {Function|Callback}
 */
_nodeV1.prototype.aeadEncrypt = aeadEncrypt;
 ...
 * @returns {Function|Callback}
 */
_nodeV1.prototype.aeadDecrypt = aeadDecrypt;
