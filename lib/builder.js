 ...
 * setIssuedAt
 *
 * Set the 'iat' claim for the token we're building.
 * 
 * @param {Date} time
 * @returns {Builder}
 */
Builder.prototype.setIssuedAt  = function (time = null) {
...
Builder.prototype.toString = function () {
	...
	switch (this.purpose) {
		case 'local':
			return this.version.encrypt(JSON.stringify(this.token.getClaims()), this.key, this.token.getFooter()).then(token => token);
		case 'public':
			return this.version.sign(JSON.stringify(this.token.getClaims()), this.key, this.token.getFooter()).then(token => token);
		default:
