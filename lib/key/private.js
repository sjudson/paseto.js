...
const PublicKey = require('./public/node');
...
function generate(cb) {
  ...
  if (self.protocol() instanceof V1) {
    return new Promise((resolve, reject) => {
      crypto.generateKeyPair('rsa', { modulusLength: 2048, privateKeyEncoding: { type: 'pkcs1', format: 'pem' } }, (err, _, rkey) => {
        if (err) { return reject.call(done, err); }
        return self.inject(rkey, (err) => { return resolve.call(done, err); });
      });
    });
  } else {
    return sodium.ready.then(() => {
      return self.inject(Buffer.from(sodium.crypto_sign_keypair().privateKey)).then((err) => { return done(err); });
    });
  }
}
...
function _public(cb) {
  ...
  if (self.protocol() instanceof V1) {
    const ko   = crypto.createPublicKey({ key: self.raw(), format: 'pem' });
    const rkey = ko.export({ type: 'pkcs1', format: 'pem' });
